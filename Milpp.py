...
# Initializing the berthing and waiting lobbies
berthing_lobby = {}
waiting_lobby = {}

# Defining the berthing capacity and
#  waiting capacity
berthing_capacity = 10
waiting_capacity = 15

# Defining the ship size mapping, berthing time, and emission index mapping
...
def ship_arrives(ship_id, ship_size, ship_priority, ship_emission_index):
    ...
    if len(berthing_lobby) < berthing_capacity:
        ...
        berthing_time_seconds = berthing_time[ship_size] * 3600  # Calculate berthing time in seconds
        ...
        if conn is not None:
            ...
            from_time = datetime.fromtimestamp(arrival_time).strftime('%Y-%m-%d %H:%M:%S')
            to_time = datetime.fromtimestamp(arrival_time + berthing_time_seconds).strftime('%Y-%m-%d %H:%M:%S')
